...
public class BoardGrid extends GridPane {
    ...
    //setting the images from resource files
    ...
    //initialize the model architecture here too
        ...
        if (model.getState() != GameState.CHECK_MATE && model.getState() != GameState.STALE_MATE) {
            ...
            node.setOnMousePressed(e -> {
                ...
                if (node.getParent() != null) {
                    node.getParent().toFront(); // Bring the parent of the ImageView to the front
                }
            });
    ...
    // Method to check if a piece has moved based on translation values
